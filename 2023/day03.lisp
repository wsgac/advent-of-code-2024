...
(defun get-number-coords (lines)
  (loop
    for row in lines
    for rownum from 0
    append (loop
             for (l r) on (re:all-matches "[0-9]+" row) by #'cddr
             collect `(:left ,l :right ,r :row ,rownum))))

(defun parse-input-to-array (lines)
  (let* ((rows (length lines))
         (cols (length (first lines))))
    (make-array `(,rows ,cols) :initial-contents lines)))

(defun part-number? (number-coords engine-array)
  (tr:match number-coords
    ((list :left left :right right :row row)
     (let* ((rows (array-dimension engine-array 0))
            (cols (array-dimension engine-array 1))
            (wishful-contour (loop
                          for col from (1- left) to right
                          collect (list (1- row) col) into points
                          collect (list (1+ row) col) into points
                          finally (return (list* (list row (1- left))
                                                 (list row right)
                                                 points))))
            (contour (remove-if-not (tr:lambda-match 
                                      ((list r c)
                                       (and (< -1 r rows)
                                            (< -1 c cols))))
                                    wishful-contour))
...
(defun problem-1 (&key (input *input-part-1-test*))
  (let* ((lines (uiop:split-string input :separator '(#\newline)))
         ...
         (part-coords (remove-if-not (a:rcurry #'part-number? engine) coords)))
    (flet ((coords->number (coords)
             (tr:match coords
