...
(defun extract-calibration-values (input extractor)
  (flet ()
   (loop
     for lastpos = 0 then (1+ pos)
     for pos = (position #\newline input :start lastpos)
     collect (funcall extractor input lastpos pos) into digits
     while pos
     finally (return digits))))

(defun get-digits (input start end)
  (list (find-if #'digit-char-p input :start start :end end)
        (find-if #'digit-char-p input :start start :end end :from-end t)))

(defun problem-1 (&key (input *input-part-1-test*))
  (loop
    for digits in (extract-calibration-values input #'get-digits)
    sum (parse-integer (coerce digits 'string))))

(defparameter *digit-to-numeral*
  (loop
    for i from 0 to 9
    for ichar = (code-char (+ i (char-code #\0)))
    collect (cons ichar (format nil "~r" i))))

(defun ensure-digit (digit-or-numeral)
  (or (car (rassoc digit-or-numeral *digit-to-numeral* :test #'string=))
      (character digit-or-numeral)))

(defparameter *digit-regex*
  (loop
    for i from 0 to 9
    collect i into digits
...
(defun problem-2 (&key (input *input-part-2-test*))
  (loop
    for digits in (extract-calibration-values input #'get-digits-or-words)
    sum (parse-integer (coerce digits 'string))))
